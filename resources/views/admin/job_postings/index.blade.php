        ...
        <div class="layout-specing">
            ...
            <div class="md:flex justify-between items-center">
                ...
                <ul class="tracking-[0.5px] inline-block sm:mt-0 mt-3">
                    ...
                    <li class="inline-block capitalize text-[14px] font-bold text-blue-600 dark:text-white" aria-current="page">Job Postings</li>
                </ul>
            </div>
            ...
            <div class="grid grid-cols-1 mt-6">
                <div class="relative overflow-x-auto block w-full bg-white dark:bg-slate-900 shadow-sm dark:shadow-gray-700 rounded-md">
                    <table class="w-full text-start">
                        <thead class="text-base">
                            <tr>
                                <th class="text-start p-4 min-w-[150px]">Job Title</th>
                                <th class="text-start p-4 min-w-[150px]">Category</th>
                                <th class="text-center p-4 min-w-[150px]">Type</th>
                                <th class="text-center p-4 min-w-[100px]">Status</th>
                                <th class="text-center p-4 min-w-[100px]">Active</th>
                                <!-- <th class="text-center p-4 min-w-[150px]">Start Date</th>
                                <th class="text-center p-4 min-w-[150px]">End Date</th> -->
                                <th class="text-end p-4 min-w-[280px]"></th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            @else
                                @foreach ($jobPostings as $jobPosting)
                                    <tr>
                                        <th class="text-start border-t border-gray-100 dark:border-gray-800 p-4">{{ $jobPosting->job_title }}</th>
                                        <td class="text-start border-t border-gray-100 dark:border-gray-800 p-4">{{ $jobPosting->job_category }}</td>
                                        <td class="text-center border-t border-gray-100 dark:border-gray-800 p-4">{{ $jobPosting->employment_type }}</td>
                                        <td class="text-center border-t border-gray-100 dark:border-gray-800 p-4">
                                            @if ($jobPosting->status === 'open')
                                                <span class="bg-green-600/10 dark:bg-green-600/20 text-green-600 text-[12px] font-bold px-2.5 py-0.5 rounded h-5 ms-1">Open</span>
                                            @else
                                                <span class="bg-blue-600/10 dark:bg-blue-600/20 text-red-600 text-[12px] font-bold px-2.5 py-0.5 rounded h-5 ms-1">Closed</span>
                                            @endif
                                        </td>
                                        <td class="text-center border-t border-gray-100 dark:border-gray-800 p-4">
                                            ...
                                            @else
                                                <span class="bg-blue-600/10 dark:bg-blue-600/20 text-red-600 text-[12px] font-bold px-2.5 py-0.5 rounded h-5 ms-1">Inactive</span>
                                            @endif
                                        </td>
                                        <!-- <td class="text-center border-t border-gray-100 dark:border-gray-800 p-4">{{ \Carbon\Carbon::parse($jobPosting->start_date)->format('jS F Y') }}</td>
                                        <td class="text-center border-t border-gray-100 dark:border-gray-800 p-4">{{ \Carbon\Carbon::parse($jobPosting->end_date)->format('jS F Y') }}</td> -->
                                        <td class="text-end border-t border-gray-100 dark:border-gray-800 p-4">
                                            <a href="{{ route('admin.job_postings.show', \Illuminate\Support\Facades\Crypt::encrypt($jobPosting->id)) }}" class="py-1 px-2 inline-block font-semibold tracking-wide border align-middle duration-500 text-sm text-center bg-blue-600 hover:bg-blue-700 border-blue-600 hover:border-blue-700 text-white rounded-md" title="View">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                                </svg>
                                            </a>
                                            <a href="{{ route('admin.job_postings.edit', \Illuminate\Support\Facades\Crypt::encrypt($jobPosting->id)) }}" class="py-1 px-2 inline-block font-semibold tracking-wide border align-middle duration-500 text-sm text-center bg-blue-600/5 hover:bg-blue-600 border-red-600/10 hover:border-red-600 text-blue-600 hover:text-white rounded-md ms-2" title="Edit">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                </svg>
                                            </a>
                                            <form action="{{ route('admin.job_postings.close', \Illuminate\Support\Facades\Crypt::encrypt($jobPosting->id)) }}" method="POST" class="inline">
                                                @csrf
                                                @method('PATCH')
                                                <button type="submit" style="cursor: pointer;" class="py-1 px-2 inline-block font-semibold tracking-wide border align-middle duration-500 text-sm text-center bg-blue-600 hover:bg-blue-700 border-blue-600 hover:border-blue-700 text-white rounded-md ms-2" title="Close">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4" />
                                                    </svg>
                                                </button>
                                            </form>
                                            <form action="{{ route('admin.job_postings.destroy', \Illuminate\Support\Facades\Crypt::encrypt($jobPosting->id)) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                ...
                <div class="mt-5 flex items-center justify-between">
                    ...
                    <span class="text-slate-400">Showing {{ $jobPostings->firstItem() }} - {{ $jobPostings->lastItem() }} out of {{ $jobPostings->total() }}</span>
                </div>
            </div>
            <!-- End Content -->
        </div>
