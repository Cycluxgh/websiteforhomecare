        ...
        <div class="layout-specing">
            ...
            <div class="grid grid-cols-1 mt-6">
                    ...
                    <table class="w-full text-start">
                        <thead class="text-base">
                            <tr>
                                ...
                                <th class="text-start p-4 min-w-[150px]">Doctor</th>
                                <th class="text-start p-4 min-w-[150px]">Company Number</th>
                                <th class="text-start p-4 min-w-[200px]">Job Title</th>
                                <th class="text-center p-4 min-w-[150px]">Date of Birth</th>
                                <th class="text-center p-4 min-w-[150px]">Submitted At</th>
                                <th class="text-end p-4 min-w-[120px]"></th>
                            </tr>
                        </thead>
                        <tbody>
                                ...
                                <tr>
                                    <td class="px-4 py-8 border-t border-gray-100 dark:border-gray-800" colspan="7">
                                        <div class="text-center text-gray-500 dark:text-gray-400">
                                            No medical self-assessments found.
                                        </div>
                                    </td>
                                </tr>
                            @else
                                @foreach ($assessments as $assessment)
                                    <tr>
                                        ...
                                        <td class="text-start border-t border-gray-100 dark:border-gray-800 p-4">{{ $assessment->doctor_name }}</td>
                                        <td class="text-start border-t border-gray-100 dark:border-gray-800 p-4">{{ $assessment->company_number }}</td>
                                        <td class="text-start border-t border-gray-100 dark:border-gray-800 p-4">{{ $assessment->job_title }}</td>
                                        <td class="text-center border-t border-gray-100 dark:border-gray-800 p-4">
                                            @if ($assessment->date_of_birth)
                                                {{ \Carbon\Carbon::parse($assessment->date_of_birth)->format('jS F Y') }}
                                            @else
                                                -
                                            @endif
                                        </td>
                                        <td class="text-center border-t border-gray-100 dark:border-gray-800 p-4">
                                            {{ \Carbon\Carbon::parse($assessment->created_at)->format('jS F Y h:i A') }}
                                        </td>
                                        <td class="text-end border-t border-gray-100 dark:border-gray-800 p-4">
                                            <a href="{{ route('admin.medical_self_assessment.show', \Illuminate\Support\Facades\Crypt::encrypt($assessment->id)) }}" class="py-1 px-2 inline-block font-semibold tracking-wide border align-middle duration-500 text-sm text-center bg-blue-600 hover:bg-blue-700 border-blue-600 hover:border-blue-700 text-white rounded-md" title="View Details">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                                </svg>
                                            </a>
                                            {{-- <form action="{{ route('admin.medical_self_assessment.destroy', \Illuminate\Support\Facades\Crypt::encrypt($assessment->id)) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" style="cursor: pointer;" class="py-1 px-2 inline-block font-semibold tracking-wide border align-middle duration-500 text-sm text-center bg-blue-600/5 hover:bg-blue-600 border-red-600/10 hover:border-red-600 text-red-600 hover:text-white rounded-md ms-2" title="Delete Medical Assessment">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="mt-5 flex items-center justify-between">
                    ...
                    <span class="text-slate-400">Showing {{ $assessments->firstItem() }} - {{ $assessments->lastItem() }} out of {{ $assessments->total() }}</span>
                </div>
            </div>
            <!-- End Content -->
        </div>
