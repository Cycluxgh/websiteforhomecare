<x-app-layout>
    <div class="layout-specing">
        <h5 class="text-lg font-semibold mb-4">Create Task Item</h5>
        ...
        <div class="p-6 bg-white dark:bg-slate-900 shadow-sm dark:shadow-gray-700 rounded-md">
            <form method="POST" action="{{ route('tasks.items.store') }}">
                @csrf
                <div class="form-group mb-3">
                    <label for="task_category_id" class="form-label font-medium">Task Category</label>
                    <select name="task_category_id" id="task_category_id"
                        ...
                        @foreach ($taskCategories as $id => $name)
                            <option value="{{ $id }}" {{ old('task_category_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group mb-3">
                    <label for="name" class="form-label font-medium">Task Item Name</label>
                    <input type="text" name="name" id="name"
                           class="form-input w-full py-2 px-3 h-10 bg-transparent dark:bg-slate-900 dark:text-slate-200 rounded outline-none border border-gray-200 focus:border-blue-600 dark:border-gray-800 dark:focus:border-blue-600 focus:ring-0"
                           value="{{ old('name') }}" placeholder="Enter task item name">
                </div>
                <div class="form-group mb-3">
